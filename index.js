var name1 = prompt("Your Name:");
var name2 = prompt("Your Partner Name:");
document.querySelector (".player1").innerHTML = name1;
document.querySelector (".player2").innerHTML = name2;

var randomNumber1 = Math.floor(Math.random ()*6)+1;
var diceRandomNum1 = "images/dice"+randomNumber1+".png";
var image1 = document.querySelectorAll ("img")[0];
image1.setAttribute("src", diceRandomNum1);

var randomNumber2 = Math.floor(Math.random()*6)+1;
var diceRandomNum2 = "images/dice"+randomNumber2+".png";
var image2 = document.querySelectorAll ("img")[1];
image2.setAttribute("src", diceRandomNum2);


if (randomNumber1>randomNumber2) {
    document.querySelector ("h1").innerHTML = name1+" Wins!";
}

else if (randomNumber2>randomNumber1) {
    document.querySelector ("h1").innerHTML = name2+" Wins!";
}

else {
    document.querySelector("h1").innerHTML = "Draw";
}

